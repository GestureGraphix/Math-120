...
\sol{
    ...
    \begin{center}
        \begin{tikzpicture}[scale=1.5]
            ...
            % Shading the region D
            \fill[green,opacity=0.3] (1,0) -- plot[domain=1:2.718] (\x,{ln(\x)}) -- (2.718,0) -- cycle;
...
\sol{
    \begin{center}
        \begin{tikzpicture}[scale=0.8]
            % Axes
            \draw[thick,->] (0,0) -- (5.5,0) node[anchor=north] {$x$};
            \draw[thick,->] (0,0) -- (0,4.5) node[anchor=east] {$y$};
            
            % Plot y = x
            \draw[domain=0:4,thick] plot (\x, \x) node[anchor=south west] {$y = x$};
            
            % Vertical line at x = 4
            \draw[thick] (4,0) -- (4,4) node[anchor=west] {$x = 4$};
            
            % Horizontal line at y = 0
            \draw[thick] (0,0) -- (4,0) node[anchor=north east] {};
            
            % Shading the region D
            \fill[blue,opacity=0.3] (0,0) -- (4,4) -- (4,0) -- cycle;
...
\sol{ \\
    ...
    \begin{center}
        \begin{tikzpicture}[scale=1]
            \begin{axis}[
                xlabel={$x$}, ylabel={$y$}, zlabel={$z$},
                xmin=0, xmax=1.5, ymin=0, ymax=1.5, zmin=0, zmax=2.5,
                axis lines=middle,
                ticks=none,
                domain=0:1,
                samples=20,
                ...
                % Surface plot z = 2 - y^2
                \addplot3[surf, shader=flat, opacity=0.5, colormap/viridis] 
                    {2 - y^2};
                
                % Line y = 1 - x on the xy-plane
                \addplot3[thick, domain=0:1] 
                    (x, 1-x, {0});
                    
                % Vertical lines at the corners
                \draw[thick] (axis cs: 1, 0, 0) -- (axis cs: 1, 0, 2);
                \draw[thick] (axis cs: 0, 1, 0) -- (axis cs: 0, 1, 2);
            \end{axis}
        \end{tikzpicture}
    \end{center}        
}
...
\sol{\\
    ...
    \begin{center}
        \begin{tikzpicture}[scale=1]
            % Axes
            \draw[->] (-0.5,0) -- (2,0) node[anchor=north]{$x$};
            \draw[->] (0,-0.5) -- (0,5) node[anchor=west]{$y$};
    
            % Lines
            \draw[thick] (0,0) -- (0,4); % x = 0 (y-axis)
            \draw[thick] (0,0) -- (1,4); % x = (1/4)y
            \draw[thick] (0,4) -- (1,4); % y = 4
    
            % Shaded region
            \fill[blue!30, opacity=0.5] (0,0) -- (0,4) -- (1,4) -- cycle;
    
            % Labels
            \node at (0.5,2) [anchor=west] {$x = \tfrac{1}{4} y$};
            \node at (0,4) [anchor=east] {$(0,4)$};
            \node at (1,4) [anchor=west] {$(1,4)$};
        \end{tikzpicture}
    \end{center}
    ...
    \begin{center}
        \begin{tikzpicture}[scale=1.5]
          ...
          % Label ticks on axes
          \draw (0,3) node[left] {$3$} -- (0.1,3);
          \draw ({sqrt(6)},0) node[below] {$\sqrt{6}$} -- ({sqrt(6)},0.1);
          \draw (3,0) node[below] {$3$} -- (3,0.1);
        
          % Draw the rectangle region for the first integral
          \fill[blue!20,opacity=0.5] (0,0) rectangle ({sqrt(6)},3);
        
          % Draw the curve y = 9 - x^2
          \draw[domain=sqrt(6):3,samples=100] plot (\x, {9 - \x*\x});
        
          % Shade the region under the curve for the second integral
          \fill[blue!20,opacity=0.5,domain=sqrt(6):3,samples=100,variable=\x]
            ({sqrt(6)},0) -- plot ({\x}, {9 - \x*\x}) -- (3,0) -- cycle;
        
          % Draw dashed lines to indicate boundaries
          \draw[dashed] ({sqrt(6)},0) -- ({sqrt(6)},3) -- (0,3);
        
          % Mark and label key points
          \draw[fill=black] ({sqrt(6)},3) circle (0.03) node[above right] {$(\sqrt{6},\, 3)$};
          \draw[fill=black] (3,0) circle (0.03) node[below right] {$(3,\, 0)$};
          \draw[fill=black] (0,3) circle (0.03) node[above left] {$(0,\, 3)$};
        
          % Label the curve
          \node at (2.2,0.7) {$y = 9 - x^2$};
        
        \end{tikzpicture}
    \end{center}
    \[  x^2 + y = 9 \implies y = 9 - x^2 \]
    \[ 0 \leq y \leq 3, \quad 0 \leq x \leq \sqrt{9 - y} \implies 0 \leq x \leq 3, \quad 0 \leq y \leq 9 - x^2 \]
    \[ \int_0^{\sqrt{6}} \int_0^3 f(x, y) \, dy \, dx + \int_{\sqrt{6}}^3 \int_0^{9 - x^2} f(x, y) \, dy \, dx \]
    c)     
    \begin{center}
        ...
        % Extend axes to x=0.5 and y=0
        \draw[dashed] (0.5,0) -- (0.5,2.5);
        
        % Label x-axis ticks
        \draw (0.5,0) -- (0.5,-0.05) node[below] {$0.5$};
        \draw (1,0) -- (1,-0.05) node[below] {$1$};
        \draw (4,0) -- (4,-0.05) node[below] {$4$};
        
        % Draw horizontal lines at y=ln2 and y=ln8
        \draw[dashed] (0,{ln(2)}) -- (4.5,{ln(2)});
        \draw[dashed] (0,{ln(8)}) -- (4.5,{ln(8)});
        
        % Label y-axis ticks
        \draw (0,{ln(2)}) -- (-0.05,{ln(2)}) node[left] {$\ln 2$};
        \draw (0,{ln(8)}) -- (-0.05,{ln(8)}) node[left] {$\ln 8$};
        
        % Plot the curve x = e^y / 2
        \draw[domain={ln(0.5)}:{ln(8)},samples=100,variable=\y,thick]
            plot ({exp(\y)/2}, {\y});
        
        % Label the curve
        \node at (1.5,1.2) [above] {$x = \dfrac{e^y}{2}$};
        
        % Fill the first region (rectangle)
        \fill[blue!20,opacity=0.5] (1,0) rectangle (4,{ln(2)});
        
        % Fill the second region
        \fill[blue!20,opacity=0.5,domain=ln(2):ln(8),variable=\y]
            plot ({exp(\y)/2}, {\y}) -- (4,{ln(8)}) -- (4,{ln(2)}) -- cycle;
        
        % Mark key points
        \draw[fill=black] (1,{ln(2)}) circle (0.03) node[below left] {$(1,\, \ln 2)$};
        \draw[fill=black] (4,{ln(8)}) circle (0.03) node[above right] {$(4,\, \ln 8)$};
        \draw[fill=black] (0.5,0) circle (0.03) node[below left] {$(0.5,\, 0)$};
        
        % Annotations
        \node at (2.5, {ln(2)/2}) {$\text{First Region}$};
        \node at (3.2, {1.5}) {$\text{Second Region}$};
        
        \end{tikzpicture}
        \end{center}
    \[
    \int_0^4 \int_0^{\ln 2x} f(x, y) \, dy \, dx
    \]

    \[
    y \leq \ln(2x) \quad \Rightarrow \quad e^y \leq 2x \quad \Rightarrow \quad x \geq \frac{e^y}{2}
    \]

    \[
    x \in \left[1, 4\right] \quad \text{for} \quad y \in \left[0, \ln 2\right]
    \]

    \[
    x \in \left[\frac{e^y}{2}, 4\right] \quad \text{for} \quad y \in \left[\ln 2, \ln 8\right]
    \]

    \[
    \int_0^{\ln 2} \int_1^4 f(x, y) \, dx \, dy + \int_{\ln 2}^{\ln 8} \int_{\frac{e^y}{2}}^4 f(x, y) \, dx \, dy
    \]
}
...
\sol{
    ...
    \[ \left. ye^{\frac{x}{y}} \right|_{0}^{y} \]  
    \[ \int_{0}^{1} ye-y dy \]
    \[ \left. \frac{ey^{2}}{2} - \frac{y^{2}}{2} \right|_{0}^{1} \] \
    \[ \frac{e-1}{2} \] 
...
\sol{
    ...
    \begin{center}
        \begin{tikzpicture}[scale=1.5]
            % Circles
            \draw[thick] (0,0) circle (1); % Inner circle
            \draw[thick] (0,0) circle (2); % Outer circle
    
            % Shaded region between the circles on the left
            \fill[blue!30, opacity=0.5]
                (0,1) arc (90:270:1) --
                (0,-2) arc (270:90:2) -- cycle;
    
            % x-axis and y-axis
            \draw[->] (-2.5,0) -- (2.5,0) node[anchor=north]{$x$}; % x-axis
            \draw[->] (0,-2.5) -- (0,2.5) node[anchor=west]{$y$}; % y-axis
    
            % Labels
            \node at (-0.7,1.2) {$x^2 + y^2 = 1$};
            \node at (-1.7,2.2) {$x^2 + y^2 = 4$};
        \end{tikzpicture}
    \end{center}    
    ...
    \begin{center}
        \begin{tikzpicture}[scale=1.5]
            % Circle
            \draw[thick] (0,0) circle (2.5); % Circle with radius 5

            % Shaded region in the first quadrant
            \fill[blue!30, opacity=0.5] (0,0) -- (2.5,0) arc[start angle=0, end angle=90, radius=2.5] -- (0,2.5) -- cycle;

            % Axes
            \draw[->] (-0.5,0) -- (2.7,0) node[anchor=north]{$x$}; % x-axis
            \draw[->] (0,-0.5) -- (0,2.7) node[anchor=west]{$y$}; % y-axis

            % Labels
            \node at (1.5,1.5) {$x^2 + y^2 = 25$};
        \end{tikzpicture}
    \end{center}
