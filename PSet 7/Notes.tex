...
\sol{\\
    ...
    b)     
    \[ \int_{0}^{3} \int_{\sqrt{9-y}}^{3} f(x,y) \, dx \, dy \]
    \[ \iint\limits_{D} f(x,y) dA \quad D = \{ (x,y) | \sqrt{4 - y} \leq x \leq 3, 0 \leq y \leq 3 \} \]
    \[ x = \sqrt{9 -y} \Rightarrow x^{2} = 9 - y \Rightarrow x^{2} - 9 = -y \Rightarrow -x^{2} + 9 = y \] 
    \[ 3 = 9 - x^{2} \Rightarrow -6 = - x^{2} \Rightarrow 6 = x^{2} \Rightarrow x = \sqrt{6} \]
    \[ \iint_{D} f(x,y) dA \quad D = \{ (x,y)| -x^{2} + 9 \leq y \leq 3, \sqrt{6} \leq x \leq 3 \} \] 
    \[ \int_{\sqrt{6}}^{3} \int_{-x^{2} + 9}^{3} f(x,y) \, dy \, dx \]  
    c) 
