...
\sol{
    ...
    c) \\
    We get an absolute maximum, but not an absolute minimum, meaning this region is neither closed nor bounded, so the Extreme Value Theorem doesn't apply here.}
...
\sol{
    ...
    \[ f(0,2) = 0^{2} - (2-2)^{2} = 0 \]
    \[ \text{Case: } x = 0 \]
    \[ 0^{2} + y^{2} = 9 \Rightarrow y = \pm 3  \quad y \geq 0 \quad y = 3 \]
    \[ f(0,2) = 0^{2} - (3-2)^{2} = - 1 \]
    \[ \text{Case: } x \neq 0 \]
    \[ 1 = \lambda \]
