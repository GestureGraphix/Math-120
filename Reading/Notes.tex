...
\defrose{Vector Field}{
	DDefintion: Let \( D \) be a set in \( \mathbb{R}^2 \) (a plane region). A \textbf{vector field on \( \mathbb{R}^2 \)} is a function \( \mathbf{F} \) that assigns to each point \( (x, y) \) in \( D \) a two-dimensional vector \( \mathbf{F}(x, y) \).
}

\defrose{3 Dimensional Vector Fields}{
	DDefintion: Let \( E \) be a subset of \( \mathbb{R}^3 \). A \textbf{vector field on \( \mathbb{R}^3 \)} is a function \( \mathbf{F} \) that assigns to each point \( (x, y, z) \) in \( E \) a three-dimensional vector \( \mathbf{F}(x, y, z) \).
}

\defrose{Gradient Fields}{
	DDefintion: If \( f \) is a scalar function of two variables, its gradient \( \nabla f \) (also called grad \( f \)) is defined by
	\[
...
\chapter{CPSC 201 Midterm 2 Review}
...
\defrose{Truth table}{
	TText: 
	\begin{enumerate}
		\item Table of all the possible trith values returned by a boolean expressoin from all the possible inputs
	\end{enumerate}
}

\defrose{Sum of Products}{
	TText:
	\begin{enumerate}
		\item Isolate rows where teh output is 1/true and ignore rows where output is 0/false
		\item For each true row, write a sub-expression that takes the AND of the all the variables together while taking the NOT for any variable whose input value is 0
	\end{enumerate}
}
