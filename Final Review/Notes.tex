...
\addcontentsline{toc}{section}{Implicit Differentiation}
...
\defrose{Fundamental Theorem for Line Integrals}{
    IIf we think of the gradient vector \( \nabla f \) of a function \( f \) of two or three variables as a derivative of \( f \), then the following theorem can be regarded as a version of the Fundamental Theorem for line integrals: \\
    ...
    \subsection*{Note 1}
    Theorem 2 states that we can evaluate the line integral of a conservative vector field (the gradient vector field of the potential function \( f \)) simply by knowing the value of \( f \) at the endpoints of \( C \). In fact, Theorem 2 says that the line integral of \( \nabla f \) is the net change in \( f \). For example:
    
    - If \( f \) is a function of two variables and \( C \) is a plane curve with initial point \( A(x_1, y_1) \) and terminal point \( B(x_2, y_2) \), then:
    \[
    \int_C \nabla f \cdot d\mathbf{r} = f(x_2, y_2) - f(x_1, y_1).
    \]
    
    - If \( f \) is a function of three variables and \( C \) is a space curve joining the point \( A(x_1, y_1, z_1) \) to the point \( B(x_2, y_2, z_2) \), then:
    \[
    \int_C \nabla f \cdot d\mathbf{r} = f(x_2, y_2, z_2) - f(x_1, y_1, z_1).
    \]
    
    \subsection*{Note 2}
    Under the hypotheses of Theorem 2, if \( C_1 \) and \( C_2 \) are smooth curves with the same initial points and the same terminal points, then:
    \[
    \int_{C_1} \nabla f \cdot d\mathbf{r} = \int_{C_2} \nabla f \cdot d\mathbf{r}.
    \]
    
    This highlights that for conservative vector fields, the line integral depends only on the endpoints of the curve, not the path taken between them.
