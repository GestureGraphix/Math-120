...
\sol{
	\\
	a) \\
	$f_{x} > 0$ going to the right increases the level curves at point P. \\
	$f_{y} > 0$ as you go up in the y direction at point p there is also an increase in the value of f. \\  
	$f_{xx} < 0 $ The level curves in teh x-direction appear to be getting closer together as we move to the right, so negative. \\
	$f_{yy} > 0$ level curves in the y-direction are spread apart implying that the slope in increasing. \\
	$f_{xy} $ ??? \\
	b)   yet to draw 
...
\sol{
	\\
	a)
	$\frac{\partial W}{\partial T}$ means that for every drop in 1 $^{\circ}$C there is a decrease if Wheat production of 2 units. \\
