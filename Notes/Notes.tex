...
\defrose{Gradient}{
	GGradient of $f(x,y)$ 
	\[ \bigtriangledown f = \langle f_{x}, f_{y} \rangle \] 
	RMK: \\
	Given a direction (unit) $\vec{u}$, we have 
	\[ D_{\vec{u}} f(x_{0}, y_{0}) = \vec{u} \cdot \langle f_{x}\left(x_{0}, y_{0}\right), f_{y}\left(x_{0},y_{0} \right)\rangle \] 
	\[ = \vec{u} \cdot \vec{v} f\left(x_{0}, y_{0}\right) \] 
}

\exrose{}{Given $\left(x_{0}, y_{0} \right)$ what is the direction that $f$ increases/decreases teh most? \\
	$f$ increases the most in the direction $\frac{\bigtriangledown f \left( x_{0}, y_{0}\right)}{|\bigtriangledown f|}$ \\
	$f$ decreases the most in the direction $\frac{- \bigtriangledown f}{|\bigtriangledown f|} \left(x_{0}, y_{0} \right)$
}
...
\exrose{}{
	\[ f(x,y) = xy - x^{2}y - xy^{2} \]
	\[ \triangledown f (x,y) = \vec{0} \] 
	\[ \langle y - 2x - y^{2}, x - x^{2} - 2xy \rangle = 0 \]
	\[ y - 2xy - y^{2} = 0 \quad x - x^{2} - 2xy = 0 \]
	\[ y(1 - 2x - y) = 0 \quad x(1-x - 2y) = 0 \]    
}
