...
\sol{
    \[
\text{(a)} \quad \vec{r}(x,\theta) = \left\langle x, 2\sin\theta, \cos\theta \right\rangle, \quad -2 \leq x \leq 2, \quad 0 \leq \theta \leq \frac{\pi}{2}
\]

\[
\text{(b)} \quad
\vec{r}_x = \left\langle 1, 0, 0 \right\rangle, \quad \vec{r}_\theta = \left\langle 0, 2\cos\theta, -\sin\theta \right\rangle, \quad \vec{N} = \vec{r}_x \times \vec{r}_\theta = \left\langle 0, \sin\theta, 2\cos\theta \right\rangle
\]

\[
\text{(c)} \quad
F_x = e^{4x^2 \sin^2\theta \cos^2\theta}, \quad F_y = 2x^2 \sin\theta, \quad F_z = \cos^2\theta e^{x/5}
\]

\[
\vec{F} \cdot \vec{N} = 2x^2 \sin^2\theta + 2\cos^3\theta e^{x/5}
\]

\[
\Phi = \int_{x=-2}^{2} \int_{\theta=0}^{\frac{\pi}{2}} \left( 2x^2 \sin^2\theta + 2\cos^3\theta e^{x/5} \right) d\theta \, dx
\]

\[
\int_{\theta=0}^{\frac{\pi}{2}} \sin^2\theta \, d\theta = \frac{\pi}{4}, \quad \int_{\theta=0}^{\frac{\pi}{2}} \cos^3\theta \, d\theta = \frac{2}{3}
\]

\[
\Phi = \int_{x=-2}^{2} \left( 2x^2 \cdot \frac{\pi}{4} + 2 \cdot \frac{2}{3} e^{x/5} \right) dx
\]

\[
\int_{x=-2}^{2} x^2 dx = \frac{16}{3}, \quad \int_{x=-2}^{2} e^{x/5} dx = 5 \left( e^{2/5} - e^{-2/5} \right)
\]

\[
\Phi = \frac{\pi}{2} \cdot \frac{16}{3} + \frac{4}{3} \cdot 5 \left( e^{2/5} - e^{-2/5} \right)
\]

\[
\Phi = \frac{8\pi}{3} + \frac{20}{3} \left( e^{2/5} - e^{-2/5} \right)
\]
...
\sol{ \\
    ...
    \[
    \vec{F}(x, y, z) = x^2 y^3 z \, \hat{i} + \sin(xyz) \, \hat{j} + xyz \, \hat{k}
    \]
    ...
    \[
    dx = -3\sin\theta \, d\theta, \quad dy = 0, \quad dz = 3\cos\theta \, d\theta
    \]
    ...
    \[
    \begin{aligned}
    F_x &= x^2 y^3 z = (3\cos\theta)^2 \cdot 3^3 \cdot (3\sin\theta) = 243\cos^2\theta\sin\theta \\
    F_y &= \sin(xyz) = \sin(9\cos\theta\sin\theta) \\
    F_z &= xyz = (3\cos\theta)(3)(3\sin\theta) = 9\cos\theta\sin\theta
    \end{aligned}
    \]
    
    \[
    \int_C \vec{F} \cdot d\vec{r} = \int_0^{2\pi} [F_x\,dx + F_z\,dz]
    \]
    
    \[
    \int_C \vec{F} \cdot d\vec{r} = \int_0^{2\pi} [-729\cos^2\theta\sin^2\theta + 27\cos^2\theta\sin\theta] \, d\theta
    \]
    
    \[
    \begin{aligned}
    I &= \int_0^{2\pi} [-729\cos^2\theta\sin^2\theta + 27\cos^2\theta\sin\theta] \, d\theta \\
    &= -729\int_0^{2\pi} \cos^2\theta\sin^2\theta \, d\theta + 27\int_0^{2\pi} \cos^2\theta\sin\theta \, d\theta \\
    &= -729 \left( \frac{\pi}{4} \right) + 0 \\
    &= -\frac{729\pi}{4}
    \end{aligned}
    \]
    
    \[
    \iint_S \text{curl} \, \vec{F} \cdot d\vec{S} = -\frac{729\pi}{4}
    \]

    b)
    \[
    \vec{F}(x, y, z) = e^x \cos z \, \hat{i} + x^2 z \, \hat{j} + xy \, \hat{k}
    \]
    ...
    \[
    d\vec{r} = dy\, \hat{j} + dz\, \hat{k} = (-\sin\theta\, \hat{j} + \cos\theta\, \hat{k})\, d\theta
    \]

    \[
    \int_C \vec{F} \cdot d\vec{r} = \int_0^{2\pi} \cos (\sin\theta) \cdot 0 \, d\theta = 0
    \]

    \[
    \iint_S \text{curl} \, \vec{F} \cdot d\vec{S} = 0
    \]
