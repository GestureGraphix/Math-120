...
\sol{
    \begin{enumerate}
        ...
        Yeah, because the cylinder has radius 3 and hiehgt of 11. The SA would be 2 $\pi$ rh which is 2$\pi$(3)(11) = $66 \pi$
    \end{enumerate}    
}
...
\sol{
    \begin{enumerate}
        \item[(a)]
        \[
        \vec{r}(x, \theta) = x\,\hat{i} + 2\cos \theta\,\hat{j} + \sin \theta\,\hat{k}, \quad x \in [-2, 2], \; \theta \in [0, \pi].
        \]
    
        \item[(b)]
        \[
        \vec{r}_x = \frac{\partial \vec{r}}{\partial x} = \hat{i}, \quad \vec{r}_\theta = \frac{\partial \vec{r}}{\partial \theta} = -2\sin \theta\,\hat{j} + \cos \theta\,\hat{k}.
        \]
        The normal vector is:
        \[
        \vec{N} = \vec{r}_x \times \vec{r}_\theta = \begin{vmatrix}
        \hat{i} & \hat{j} & \hat{k} \\
        1 & 0 & 0 \\
        0 & -2\sin \theta & \cos \theta
        \end{vmatrix} = 0\,\hat{i} - \cos \theta\,\hat{j} - 2\sin \theta\,\hat{k}.
        \]
        The \( z \)-component of \( \vec{N} \) is \( N_z = -2\sin \theta \). Since \( \sin \theta \geq 0 \) for \( \theta \in [0, \pi] \), \( N_z \leq 0 \), meaning the normal vectors point downward. 
    
        To match the upward orientation, take:
        \[
        \vec{N} = -(\vec{r}_x \times \vec{r}_\theta) = 0\,\hat{i} + \cos \theta\,\hat{j} + 2\sin \theta\,\hat{k}.
        \]
        So, no. 
