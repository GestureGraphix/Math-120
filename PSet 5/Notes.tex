...
\sol{
    ...
    \[ \vec{w} = \frac{\vec{v}}{|v|} = \frac{\langle -1, -2, 2 \rangle}{\sqrt{(-1)^{2} + (-2)^{2} + 2^{2}}} = \left\langle - \frac{1}{3}, -\frac{2}{3}, \frac{2}{3}\right\rangle \] 
    \[ D_{\vec{w}}f = \left\langle 1, \frac{3}{2}, \frac{1}{2} \right\rangle \cdot \vec{w} = - \frac{1}{3} - 1 + \frac{1}{3} = - 1 \]
...
\sol{ 
    ...
    c) 
    \[ \nabla f(5,-1) = \langle -0.1, 0.08 \rangle \] 
    \[ \left|\nabla f(5,-1)\right| = \sqrt{ (-0.1)^{2} + (0.08)^{2}} = \sqrt{0.01 + 0.0064} = \sqrt{0.0164}\]
    \[ \tan \theta = \frac{\nabla f(5,-1)}{1} \approx 0.128 \] 
    \[ \arctan \left(\frac{\nabla f(5,-1)}{1}\right) \approx 7.3 \]  
}

\qs{}{
    We have seen that if a surface $S$ is defined by the equation $f(x, y, z) = k$, 
    and $(a, b, c)$ is a point on $S$, then the gradient vector $\nabla f(a, b, c)$ is 
    perpendicular to $S$ at that point. Use this property to find an equation of the 
    tangent plane to the surface $x^2 + xy^3 - z = 3$ at the point $(2, 2, 17)$.
}
...
\sol{
    \\
    Critical points at (2,3) and (0,-1) because here there are closed loops. There are also 
    critical points at (0,3) and (2, -1) because here you can see the values of the contour diverge and split in different directions. 
    The points (2,3) and (0,-1) should be local maxes because they are closed loops where the contour values seem to increase towarsd the center of the loop. 
    The other points (0,3) and (2,-1) should be saddle points because it has opposite concavity near them.  
    ...
    Point (0,-1) 
    \[ D = D(0,-1) = f_{xx}(0,3)f_{yy}(0,-1) - [f_{xy}(0,-1)]^{2}\]
    \[ D = (6(0)-6)(6(3)-6) - 0^{2} \] 
    \[ (6)(-12) = 72 \] 
    local max \\ 
    \\
    Point (0,3) 
    \[ D = D(0,3) = f_{xx}(0,3)f_{yy}(0,3) - [f_{xy}(0,3)]^{2}\]
    \[ D = (6(0)-6)(6(3)-6) - 0^{2} \] 
    \[ D = (-6)(12) = -72 \] 
    saadle point  \\ 
    \\
    Point (2,-1) 
    \[ D = D(2,-1) = f_{xx}(2,-1)f_{yy}(2,-1) - [f_{xy}(2,-1)]^{2}\]
    \[ D = (6(2)-6)(6(-1)-6) - 0^{2} \]  
    \[ (6)(-12) = -72 \] 
    saddle point \\
    \\ 
    Point (2,3) 
    \[ D = D(2,3) = f_{xx}(2,3)f_{yy}(2,3) - [f_{xy}(2,3)]^{2}\]
    \[ D = (6(2)-6)(6(3)-6) - 0^{2} \]
    \[ (6)(12) = 72 \]   
    local max 
...
\sol{ \\
    a) 
    \[ D = D(0,2) = (-1)(1) - [6]^{2} = 1 - 36 = - 35 \]
    D $<$ 0 so this is a saddle point. 
    b) 
    \[ D = D(0,2) = (-1)(-8) - [2]^{2} = 8 - 4 = 4 \]
    Since D $>$ 0 and $g_{xx}(0,2) < 0$, the point is (0,2) is a local maximum.
    c)
    \[ D = D(0,2) = (4)(9) - [6]^{2} = 26 - 26 = 0 \]   
    D = 0 menas the second derivative test is inconclusive. We do not what is going on at this point. 
}
